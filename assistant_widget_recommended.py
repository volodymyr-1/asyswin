"""
Виджет-ассистент с рекомендациями скриптов
Показывает 3 наиболее вероятных скрипта для запуска
"""
...
class AssistantWidgetRecommended(BaseAssistantWidget):
    """Виджет с рекомендациями скриптов"""

    def __init__(self):
        super().__init__(width=360, height=500, title="AsysWin Assistant")
    ...
    def _add_header_content(self, parent):
        ...
        title_label = CTkLabel(
            parent, text="AsysWin Assistant", font=CTkFont(size=14, weight="bold")
        )
    ...
    def _show_no_recommendations(self):
        ...
        msg_label = CTkLabel(
            no_rec_frame,
            text="Нет доступных скриптов для рекомендации\nВыполните запись и анализ для создания скриптов",
            font=CTkFont(size=11),
            justify="center",
        )
    ...
    def _create_script_card(self, script, position: int):
        ...
        stats_label = CTkLabel(
            stats_frame,
            text=f"⚡ {success_rate:.0f}% успеха | {script.run_count} запусков",
            font=CTkFont(size=9),
    ...
    def _on_script_run(self, script):
        ...
        self.robot_say(f"Запускаю скрипт: {script.name}")
    ...
    def _execute_script_thread(self, script):
        try:
            ...
            if result.success:
                self._update_status("ready", "Скрипт выполнен успешно", "#00ff00")
                self.robot_say("Скрипт выполнен успешно!")
            else:
                self._update_status("error", "Ошибка выполнения", "#ff4444")
                self.robot_say("Ошибка при выполнении скрипта")

        except Exception as e:
            self._update_status("error", "Ошибка запуска", "#ff4444")
            self.robot_say("Ошибка запуска скрипта")
