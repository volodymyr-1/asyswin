...
class ScriptManager:
    ...
    def __init__(self, scripts_dir: str = "generated_scripts"):
        ...
        self.execution_history_file = os.path.join(scripts_dir, "execution_history.json")
    ...
    def _calculate_script_score(self, script: ScriptInfo) -> float:
        ...
        recency_score = 0.0
        if script.last_run:
            last_run_time = datetime.fromisoformat(script.last_run.replace('Z', '+00:00'))
    ...
    def _get_time_relevance_bonus(self, script: ScriptInfo) -> float:
        ...
        for record in script_history:
            run_time = datetime.fromisoformat(record['timestamp'].replace('Z', '+00:00'))
    ...
    def _analyze_script(self, script_path: str) -> Optional[ScriptInfo]:
        ...
        try:
            ...
            last_run = None
            if history:
                last_run = sorted(history, key=lambda x: x['timestamp'], reverse=True)[0]['timestamp']
    ...
    def execute_script(self, script_path: str, timeout: int = 300, parameters: Dict = None) -> ExecutionResult:
        ...
        try:
            ...
            self._save_execution_result(script_path, success, duration, stdout, stderr, exit_code)
    ...
    def _save_execution_result(self, script_path: str, success: bool, duration: float, 
        ...
        if len(self.execution_history[script_path]) > 100:
            self.execution_history[script_path] = self.execution_history[script_path][-100:]
