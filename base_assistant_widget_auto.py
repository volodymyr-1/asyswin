"""
Base class for AsysWin assistant widgets (auto mode)
Contains common logic: UI, drag, status, callbacks
Record/Analysis buttons removed - everything works automatically
"""
...
class BaseAssistantWidget:
    """Base class for all assistant widgets (auto mode)"""

    def __init__(
        self, width: int = 320, height: int = 420, title: str = "AsysWin Assistant"
    ):
    ...
    def _create_status_area(self, parent):
        status_frame = ctk.CTkFrame(
            parent, corner_radius=10, fg_color=("gray90", "gray20"), height=70
        )
        status_frame.pack(fill="x", padx=10, pady=5)
        status_frame.pack_propagate(False)

        indicator_row = ctk.CTkFrame(status_frame, fg_color="transparent")
        indicator_row.pack(fill="x", padx=10, pady=(10, 5))

        self.status_indicator = ctk.CTkLabel(
            indicator_row, text="●", font=ctk.CTkFont(size=20), text_color="#00ff00"
        )
        self.status_indicator.pack(side="left", padx=(5, 10))

        self.status_label = ctk.CTkLabel(
            indicator_row, text="Ready", font=ctk.CTkFont(size=13), anchor="w"
        )
        self.status_label.pack(side="left", fill="x", expand=True)

        hint_label = ctk.CTkLabel(
            status_frame,
            text="Auto record on activity",
            font=ctk.CTkFont(size=9),
            text_color=("gray50", "gray60"),
            anchor="w",
        )
        hint_label.pack(fill="x", padx=15, pady=(0, 5))
    ...
    def set_recording(self, is_recording: bool):
        ...
        if is_recording:
            self._update_status("recording", "Recording actions...", "#ff4444")
            if self.robot:
                self.robot.set_state("recording", "Recording actions...")
        else:
            self._update_status("ready", "Ready", "#00ff00")
            if self.robot:
                self.robot.set_state("idle", "Ready!")
    ...
    def set_analyzing(self, is_analyzing: bool):
        ...
        if is_analyzing:
            self._update_status("analyzing", "Analyzing via Gemini...", "#ffaa00")
            if self.robot:
                self.robot.set_state("thinking", "Analyzing actions...")
        else:
            self._update_status("ready", "Ready", "#00ff00")
    ...
    def notify_analysis_complete(self, scripts_count: int):
        ...
        self._update_status("ready", "Ready", "#00ff00")
        if self.robot:
            self.robot.set_state("happy", f"Done! Created {scripts_count} scripts!")
    ...
    def notify_predictions_ready(self, count: int):
        ...
        if self.robot:
            self.robot.set_state("happy", f"Found {count} scripts!")
    ...
    def notify_error(self, message: str):
        ...
        if self.robot:
            self.robot.set_state("error", "Error occurred!")
