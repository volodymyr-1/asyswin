...
class ActionRecorder:
    ...
    def start_recording(self):
        ...
        if self.is_recording:
            print("[RECORDER] Запись уже идёт")
            return
    ...
    def stop_recording(self):
        ...
        if not self.is_recording:
            print("[RECORDER] Запись не активна")
            return
        ...
        print(f"[RECORDER] ⏹ Запись остановлена. Действий: {len(self.actions)}")
    ...
    def _save_log(self):
        ...
        if not self.actions:
            print("[RECORDER] Нет действий для сохранения")
            return None
        ...
        print(f"[RECORDER] 💾 Лог сохранён: {filepath}")
        return filepath
    ...
    def _save_semantic_logs(self):
        ...
        try:
            ...
            print(f"[RECORDER] 📊 Семантические логи сохранены:")
            print(f"   Сырой лог: {raw_path} ({summary['raw_count']} действий)")
            print(f"   Фильтрованный лог: {filtered_path} ({summary['filtered_count']} действий)")
            print(f"   Фильтрация: {summary['filtered_ratio']:.1f}%")
            
        except Exception as e:
            print(f"[RECORDER] ❌ Ошибка сохранения семантических логов: {e}")
    ...
    def get_actions_summary(self):
        ...
        summary = {
            ...
            "mouse_clicks": len([a for a in self.actions if a["type"] == "mouse_click"]),
